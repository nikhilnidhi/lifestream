        ...
        if (token) {
            try {
                ...
                if (user) {
                    ...
                    var Security = await UserModel.findOne({ _id: req.user.id, role: 'Security', status: { $ne: "Deleted" } }, { password: 0 });
                    // var tok = await tokenModel.findOne({ userid: req.user.id, tokenvalue: token, status: 'active' });
                    if ((!Mics.isnullorempty(Security)) /* && (!Mics.isnullorempty(tok)) */) {
                        req.user.user = Security;
                        next();
                    }
                    else {
                        res.status(200).json({
